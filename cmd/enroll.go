...
func init() {
        ...
        enrollCmd.Flags().StringVar(&enrollValidity, "validity", "", "Certificate validity period (formats: 30d, 6m, 1y, 30d6m, 1y6m)")
...
func runEnroll(cmd *cobra.Command, args []string) error {
        ...
        var validityPeriod *policyselect.ValidityPeriod
        if enrollValidity != "" {
                validityPeriod, err = policyselect.ParseValidity(enrollValidity)
                if err != nil {
        ...
        // Convert validity period to API format
        var apiValidity *api.ValidityPeriod
        if validityPeriod != nil {
                apiValidity = &api.ValidityPeriod{
        ...
        requestID, err := client.SubmitCSR(string(csrPEM), policyID, apiValidity)
        if err != nil {
...
Certificate Subject (Distinguished Name):
      ...
      --locality string    Locality/City (L) (default "Detroit")
      --org strings        Organization (O) (repeatable) (default [OmniCorp])
      --ou strings         Organizational Unit (OU) (repeatable: --ou "IT Department" --ou "Security Team") (default [Cybernetics])
      --province string    State/Province (ST) (default "Michigan")
...
func getEnrollUsageFunc() func(*cobra.Command) error {
        return func(cmd *cobra.Command) error {
                ...
                fmt.Printf("      --locality string    Locality/City (L) (default \"Detroit\")\n")
                fmt.Printf("      --org strings        Organization (O) (repeatable) (default [OmniCorp])\n")
                fmt.Printf("      --ou strings         Organizational Unit (OU) (repeatable: --ou \"IT Department\" --ou \"Security Team\") (default [Cybernetics])\n")
                fmt.Printf("      --province string    State/Province (ST) (default \"Michigan\")\n\n")
                ...
                fmt.Printf("      --no-key-output        Don't output private key to file\n")
