...
#   or zcert -config zcert.cnf -profile p12 --cn "mycert.com"

[Default]
# Default profile used when no -profile is specified
url = https://api.ztpki.venafi.com
...
[p12]
# Profile for PKCS#12 certificates
url = https://api.ztpki.venafi.com
...
url = https://test-api.ztpki.venafi.com
key-id = test-key-id
secret = test-secret
algo = sha256
format = pem
...
url = https://prod-api.ztpki.venafi.com
key-id = prod-key-id
secret = prod-secret
algo = sha256
format = pem
policy = ProductionWebServer
key-size = 4096
key-type = rsa
chain = true
no-key-output = false
